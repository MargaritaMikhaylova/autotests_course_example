...
class PublicTransport:
    ...
    def info(self):
        '''
        Свойство выводит на печать информацию о: марке, цвете, годе выпуска и мощности двигателя
        :return: Информация об экземпляре
        '''
        return self.brand, self.color, self.year, self.engine_power
class Bus(PublicTransport):
    ...
    def park(self):
        '''
        Свойство возвращает значение парка
        :return: Парк
        '''
        return self.__park

    @park.setter
    def park(self, number_park):
        '''
        Проверка при присвоении номера парка, что он в диапазоне от 1000 до 9999
        :param number_park: номер парка
        :return: результат проверки
        '''
        assert number_park > 1000 and number_park < 9999
        self.__park = number_park
...
class Tram(PublicTransport):
    ...
    def how_long(self):
        '''
        Свойство вычисляет время прохождения маршрута по формуле max_speed/(4*path)
