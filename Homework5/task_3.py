...
def everything_for_your_cat(cats_data):
    '''
    Функция оптимизирует данные, чтобы для каждого владельца при выводе (одна строка) данные о всех его животных
    отображались в одной строке, на новой строке следующий покупатель и все его питомцы и т.д.
    :param cats_data: Данные о кошках и их владельцах в виде списка кортежей,
    где каждый кортеж состоит из: Кличка котика, Возраст котика, Имя покупателя, Фамилия покупателя.
    ...
    for cat_info in cats_data:
        name_cat, age_cat, name_cust, surname_cust = cat_info
        if (name_cust, surname_cust) in customers:
            customers[(name_cust, surname_cust)].append((name_cat, age_cat))
        else:
            customers[(name_cust, surname_cust)] = [(name_cat, age_cat)]
    for cust in customers:
        name_cust, surname_cust = cust
        all_cats = ''
        #    for name_cat, age_cat in customers[]:
        all_cats = "; ".join([f'{name_cat}, {age_cat}' for name_cat, age_cat in customers[cust]])
        our_str += f'{name_cust} {surname_cust}: {all_cats}\n'
    return our_str
